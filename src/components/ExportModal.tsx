    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-[#151619] border border-white/10 rounded p-6 w-96 max-w-full shadow-2xl flex flex-col gap-6" data-testid="export-modal">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-white/30 hover:text-white transition-colors text-[10px] font-mono">CLOSE</button>
        </div>
        
        <div className="space-y-3">
          <h3 className="hardware-label">Select content</h3>
          <div className="max-h-40 overflow-y-auto bg-black/40 p-2 rounded border border-white/5 no-scrollbar">
            {models.length === 0 ? (
              <p className="text-[9px] text-white/20 uppercase font-mono tracking-widest italic text-center py-4">No content available to export</p>
            ) : (
              models.map(model => (
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            {recommendedFormat && <span className="text-[9px] text-blue-300 font-mono uppercase" data-testid="recommended-export-format">Best for this project: {recommendedFormat}</span>}
          </div>
          ...
          >
            {exportFormats.includes('original') && <option value="original">Original files (.zip)</option>}
            {exportFormats.includes('glb') && <option value="glb">GLB (.glb)</option>}
            {exportFormats.includes('obj') && <option value="obj">OBJ (.obj)</option>}
          ...
          {isSwimManifest && (
            <p className="text-[10px] text-blue-200/80">
              SWIM26 Manifest exports authored scene data and runtime handoff metadata.
            </p>
          )}
        </div>
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-2 cursor-pointer group">
              <input 
                ...
                className="w-3 h-3 rounded bg-black/60 border-white/10 text-white/70 focus:ring-0 focus:ring-offset-0 accent-white/50"
              />
              <span className="text-[10px] font-mono text-white/40 group-hover:text-white/70 uppercase tracking-tight">Include materials</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer group">
              <input 
                type="checkbox" 
              ...
              />
              <span className="text-[10px] font-mono text-white/40 group-hover:text-white/70 uppercase tracking-tight disabled:opacity-20">Include textures</span>
            </label>
          </div>
        </div>
