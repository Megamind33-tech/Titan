...
interface ExportModalProps {
  ...
  allowedFormats?: ('original' | 'glb' | 'obj' | 'swim26-manifest')[];
  recommendedFormat?: 'original' | 'glb' | 'obj' | 'swim26-manifest';
  contextNote?: string;
}
...
export default function ExportModal({ 
  ...
  models, 
  onExport,
  allowedFormats = ['original', 'glb', 'obj'],
  recommendedFormat = 'original',
  contextNote
}: ExportModalProps) {
  const [selectedIds, setSelectedIds] = useState<string[]>(models.map(m => m.id));
  const [format, setFormat] = useState<'original' | 'glb' | 'obj' | 'swim26-manifest'>(recommendedFormat);
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-[#151619] border border-white/10 rounded p-6 w-96 max-w-full shadow-2xl flex flex-col gap-6">
        ...
        <div className="space-y-3">
          ...
          >
            {allowedFormats.includes('original') && <option value="original">ORIGINAL_FILES (ZIP)</option>}
            {allowedFormats.includes('glb') && <option value="glb">GLB (BINARY_GLTF)</option>}
            {allowedFormats.includes('obj') && <option value="obj">OBJ (WAVEFRONT)</option>}
            {allowedFormats.includes('swim26-manifest') && <option value="swim26-manifest">SWIM26_MANIFEST (JSON)</option>}
          </select>
        </div>
